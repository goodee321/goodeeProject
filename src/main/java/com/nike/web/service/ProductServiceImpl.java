...
//			ProductDTO product = productMapper.selectProductByNo(proNo);
			...
			for(MultipartFile multipartFile : files) {
				...
				try {
					...
					if(multipartFile != null && multipartFile.isEmpty() == false) {
						...
						// 저장경로
						String riPath = FileUtils.getTodayPath();
...
//									.riNo(result)
		...
		public void deleteReview(HttpServletRequest request, HttpServletResponse response) {
			...
			Integer reviewNo = Integer.parseInt(opt.orElse("0"));
//			Integer proNo = Integer.parseInt(request.getParameter("proNo"));
			
			// 저장되어 있는 첨부 파일 목록 가져오기
			List<ReviewImageDTO> attaches = productMapper.selectFileAttachListInTheGallery(reviewNo);
			
			// 저장되어 있는 첨부 파일이 있는지 확인
			if(attaches != null && attaches.isEmpty() == false) {
				
				// 하나씩 삭제
				for(ReviewImageDTO attach : attaches) {
				
					// 첨부 파일 알아내기
					File file = new File(attach.getRiPath(), attach.getRiName());
					...
					try {
						...
						if(contentType.startsWith("image")) {
							...
							// 썸네일 이미지 삭제
							File thumbnail = new File(attach.getRiPath(), "s_" + attach.getRiName());
							if(thumbnail.exists()) {
								thumbnail.delete();
							}
			...
			int result = productMapper.deleteDetailReview(reviewNo);
		...
		// 리뷰 상세보기
		@Override
		public void findDetailReviewByNo(HttpServletRequest request, Model model) {
			
			Integer reviewNo = Integer.parseInt(request.getParameter("reviewNo"));
			model.addAttribute("reviewbyno", productMapper.selectReviewByNo(reviewNo));
			model.addAttribute("reviewNo", reviewNo);
			// 첨부 파일 정보 가져와서 model에 저장하기
			model.addAttribute("fileAttaches", productMapper.selectFileAttachListInTheGallery(reviewNo));
		}
		...
		public void changeReview(MultipartHttpServletRequest multipartRequest, HttpServletResponse response) {
			...
			// 전달된 파라미터
			Integer reviewNo = Integer.parseInt(multipartRequest.getParameter("reviewNo"));
			String reviewTitle = multipartRequest.getParameter("reviewTitle");
			String reviewContent = multipartRequest.getParameter("reviewContent");
			Integer reviewStar = Integer.parseInt(multipartRequest.getParameter("reviewStar"));
			...
			// Gallery UPDATE 수행
			int reviewResult = productMapper.updateReview(review);  // UPDATE 수행
...
//									.reviewNo(review.getReviewNo())
			...
			try {
				response.setContentType("text/html");
				PrintWriter out = response.getWriter();
				if(reviewResult == 1 && fileAttachResult == files.size()) {
					out.println("<script>");
		...
		@Override
		public ResponseEntity<byte[]> displayReview(Integer riNo, String type) {
			// 보내줘야 할 이미지 정보(path, saved) 읽기
			ReviewImageDTO reviewImage = productMapper.selectReviewImageByNo(riNo);
		
			// 보내줘야 할 이미지	
			File file = null;
			switch(type) {
			case "thumb":
				file = new File(reviewImage.getRiPath(), "s_" + reviewImage.getRiName());
				break;
			case "image":
				file = new File(reviewImage.getRiPath(), reviewImage.getRiName());
		...
		@Override
		public void removeFileAttach(Integer riNo) {
			
			// fileAttachNo가 일치하는 FileAttachDTO 정보를 DB에서 가져오면
			// 삭제할 파일의 경로와 이름이 있다.
			ReviewImageDTO fileAttach = productMapper.selectReviewImageByNo(riNo);
			
			// 첨부 파일 알아내기
			File file = new File(fileAttach.getRiPath(), fileAttach.getRiName());
			...
			try {
				...
				if(contentType.startsWith("image")) {
					...
					// 썸네일 이미지 삭제
					File thumbnail = new File(fileAttach.getRiPath(), "s_" + fileAttach.getRiName());
					if(thumbnail.exists()) {
						thumbnail.delete();
					}
			...
			// FILE_ATTACH 테이블의 ROW 삭제
			productMapper.deleteFileAttach(riNo);
